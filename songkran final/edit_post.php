        ...
        <div class="custom-expanded u-form u-form-1">
            ...
            <?php
                if (isset($_SESSION['username'])) {
                    ...
                    $joinpostwithuser = "SELECT * FROM post p INNER JOIN users u ON p.Auth_ID = u.UserID WHERE p.postID='$postID' ORDER BY time_post DESC;";
                    $result = $conn->query($joinpostwithuser);

                    if ($result->num_rows > 0) {
                        // แสดงข้อมูล
                        while ($row = $result->fetch_assoc()) {
                            echo '<form action="./edit_post_data.php" method="POST">
                <div class="u-form-group u-form-name u-label-top u-form-group-1">
                    <label for="name-3b9a" class="u-label">หัวข้อ</label>
                    <input type="text" placeholder="หัวข้อ" id="name-3b9a" name="title" class="u-input u-input-rectangle" required="" value=' . $row["title"] . '>

                </div>
                <div class="u-form-group u-form-message u-label-top">
                    <label for="message-3b9a" class="u-label">เนื้อหา</label>
                    <textarea placeholder="เนื้อหา" rows="4" cols="50" id="message-3b9a" name="content"  class="u-input u-input-rectangle" required="">' . $row["content"] . '</textarea>

                </div>
                <div class="u-align-left u-form-group u-form-submit u-label-top">
                    <input type="hidden" name="postID" value= '.$postID.' >
                    <input type="submit" value="edit" class="btn btn-primary" >
                    <a href="./delete_post_data.php?postID='.$postID.'" class="btn btn-danger" >delete</a>
